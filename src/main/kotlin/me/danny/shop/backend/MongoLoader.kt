...
class MongoLoader(options: MongoOptions) : ShopBackend {
    ...
    init {
        val uri = ConnectionString("mongodb+srv://${options.user}:${options.password}@${options.host}/")
    ...
    override fun loadShop(plugin: Plugin): LoadResult {
        val categories = database.getCollection("categories")
            .find()
            .map {
        ...
        val items = database.getCollection("items")
            .find()
            .map {
                plugin.logger.info(" ")
                ...
                val item = when (itemType) {
                    "material" -> ItemType.Mat(Material.matchMaterial(itemObj) ?: CHEST)
    ...
    override fun saveShop(plugin: Plugin, shop: Shop) {
        ...
        categories.insertMany(shop.categories()
            .map {
                Document(
                    mapOf(
                        "cid" to it.cid.id,
